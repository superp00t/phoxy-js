...
function ZDBAuthentication(o) {
  ...
  this.api = BuildURL(apiurl);
  this.username = username;
  this.password = password;
}

ZDBAuthentication.prototype.logout = function() {
  return new etc.Req({
    url:    $this.api.subPath("/v1/zdb/logout").setQ("t", this.token).toString(),
    method: "GET"
  }).do();
}

/**
 * Returns a Promise, resolving to an opcode integer.
 * 
...
/**
 * Request authorization status of a token
 * 
 * @param  {String} token 
 * @return {Promise<Number>}
 */
ZDBAuthentication.prototype.checkStatus = function(token) {
  return new etc.Req({
    method: "GET",
    url:    this.api.subPath(`/v1/zdb/checkStatus`).setQ("t", token)
  }).do()
  .then(function(r) {
    return new etc.Buffer(r.data).readInt();
  });
}
