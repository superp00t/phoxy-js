...
const StoreController = require("./Crypto/MemStore");
...
function Phoxy(o) {
  ...
  $this.url    = new etc.URL($this.apiurl);
  if (o.store) {
...
Phoxy.prototype.queryCleavePackages = function(type, name) {
  var $this = this;
  var url = $this.url.subPath(`/v1/cleave/pkg/${encodeUriCompnent(type)}`);

  if (name) {
    url = url.setQ("name", name);
  }
